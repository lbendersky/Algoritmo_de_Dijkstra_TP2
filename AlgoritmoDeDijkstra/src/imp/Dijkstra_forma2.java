...
public class Dijkstra_forma2 implements GrafoTDA {
    ...
    @Override
    public void inicializarGrafo() { // Inicializa el grafo como vacío
        origen = null;
    }

    @Override
    public void agregarVertice(int v) { // Agrega un nuevo vértice si no existe ya en el grafo
        if (vertice2nodo(v) != null)
            return;
    ...
    @Override
    public void eliminarVertice(int v) { // Elimina un vértice y todas sus aristas del grafo
    ...
    @Override
    public ConjuntoTDA vertices() { // Devuelve un conjunto con todos los vértices del grafo
    ...
    @Override
    public void agregarArista(int origen, int destino, int peso) { // Agrega una arista desde 'origen' hacia 'destino' con el peso dado
    ...
    @Override
    public void eliminarArista(int origen, int destino) {// Elimina la arista desde 'origen' hacia 'destino'
        NodoVertice o = vertice2nodo(origen);
        eliminarAristaEnVertice(o, destino);
    }

    @Override
    public boolean existeArista(int origen, int destino) { // Verifica si existe una arista desde 'origen' hacia 'destino'
        NodoVertice o = vertice2nodo(origen);
        if (o == null)
            return false;
    ...
    @Override
    public int pesoArista(int origen, int destino) { // Devuelve el peso de la arista entre 'origen' y 'destino'
        NodoVertice o = vertice2nodo(origen);
        if (o == null)
            return -1;
    ...
    private NodoVertice vertice2nodo(int v) { // Devuelve el nodo del vértice dado, o null si no existe
        NodoVertice turista = origen;
    ...
    private void eliminarAristaEnVertice(NodoVertice vertice, int destino) { // Elimina la arista hacia 'destino' desde  el nodo dado
    ...
    public ConjuntoTDA vecinos(int v) { // Devuelve un conjunto con los vecinos del vértice v
    ...
    public class Dijkstra {
        public static ConjuntoTDA copiarConjunto(ConjuntoTDA original) { // Devuelve una copia del conjunto original sin modificarlo
        ...
        public static Dijkstra_forma2 segundoMetodo(int origen, GrafoTDA grafo) { // Implementación del algoritmo de Dijkstra
            ...
            while (!todosVisitados(vertices, visitado)) {
                ...
                if (u == -1)
                    break; // Si no quedan vértices alcanzables, termina
        ...
        private static boolean todosVisitados(ConjuntoTDA vertices, boolean[] visitado) { // Verifica si todos los vértices del conjunto han sido visitados
        ...
        private static int verticeMinDistNoVisitado(ConjuntoTDA vertices, int[] distancias, boolean[] visitado) { // Devuelve el vértce no visitado con menor distancia.
